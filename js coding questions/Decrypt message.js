...
function decrypt(word) {
    ...
    for (let i = 0; i < word.length; i++){
        let asciiletter = word[i].charCodeAt(0);
        asciiletter = asciiletter - secondStep;

        while (asciiletter < 97) {
            asciiletter += 26;
        }

        decryption += String.fromCharCode(asciiletter);
        secondStep += asciiletter;
    }
    return decryption;
}
